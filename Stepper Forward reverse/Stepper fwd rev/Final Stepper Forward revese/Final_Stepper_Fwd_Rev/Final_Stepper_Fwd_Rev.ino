...
void setup() {
  ...
  Status5 = EEPROM.read(M1mmaddr, &M1mmDATA);
  M2cycle = HoldDATA;
  timedelay = PressDATA;
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    Status5 = EEPROM.update(M1mmaddr, mm_step);
...
//mm_step=5;
  ...
  stepper_speed_map = map(stepper_speed, 0, 100, 400, 100);
